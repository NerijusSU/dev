...
console.log("Visi 'users', kurie turi augintinį:", filterDogOwners(users));

//2. Užduotis
function filterAdults(users) {
  return users.filter(users => users.age >= 18);
  //return users.filter(x => x.age >= 18).map(person => person.name); // jei tik Vardas ir Pavardė 
}
console.log("Visi 'users', kurie yra pilnamečiai:", filterAdults(users));